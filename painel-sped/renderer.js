...
/* ---- Resolução de Responsável (ADD10) ----------------------------------
 * Descobre o responsável de um SPED a partir das empresas do Supabase
 * (cachedEmpresasFila). Prioridade de identificação: id exato > cnpj >
 * nome_base > nome. Nunca inventa: match ambíguo => "Sem responsável". */
const SEM_RESPONSAVEL = 'Sem responsável';
function _normTxt(v) { return (v == null ? '' : String(v)).trim().toLowerCase(); }
function _normCnpj(v) { return (v == null ? '' : String(v)).replace(/\D/g, ''); }
function _respDaEmpresa(e) { const r = e && (e.responsavel_atual || e.responsavel); return r ? String(r).trim() : ''; }

// Índice das empresas do Supabase por id, cnpj, nome_base e nome.
function buildEmpresaIndex() {
  const idx = { porId: new Map(), porCnpj: new Map(), porBase: new Map(), porNome: new Map(), pronto: false };
  const empresas = (cachedEmpresasFila && cachedEmpresasFila.empresas) || [];
  if (!empresas.length) return idx; // ainda carregando / sem dados
  idx.pronto = true;
  const add = (map, key, emp) => { if (!key) return; if (!map.has(key)) map.set(key, []); map.get(key).push(emp); };
  empresas.forEach((e) => {
    add(idx.porId, String(e.id), e);
    add(idx.porCnpj, _normCnpj(e.cnpj), e);
    add(idx.porBase, _normTxt(e.nome_base), e);
    add(idx.porNome, _normTxt(e.nome), e);
  });
  return idx;
}

// Resolve o responsável de um item de SPED. Retorna null enquanto os dados do
// Supabase ainda não chegaram (estado de carregamento → exibe "?" temporário).
function resolveResponsavel(item, idx) {
  // 1. valor já presente no próprio item (responsavel_atual tem prioridade)
  const direto = item.responsavel_atual || item.responsavel;
  if (direto && String(direto).trim()) return String(direto).trim();
  if (!idx || !idx.pronto) return null; // dados ainda carregando

  // 2. casar com a empresa pelo melhor identificador disponível
  const tentativas = [
    ['id', idx.porId, item.empresaId != null ? String(item.empresaId) : ''],
    ['cnpj', idx.porCnpj, _normCnpj(item.cnpj)],
    ['nome_base', idx.porBase, _normTxt(item.nomeBase)],
    ['nome', idx.porNome, _normTxt(item.nome)],
  ];
  for (const [chave, map, valor] of tentativas) {
    if (!valor) continue;
    const cands = map.get(valor);
    if (!cands || !cands.length) continue;
    const resps = [...new Set(cands.map(_respDaEmpresa).filter(Boolean))];
    if (resps.length === 1) return resps[0];
    if (resps.length > 1) { // mesma chave aponta para responsáveis diferentes
      console.warn(`[responsavel] match ambíguo por ${chave}="${valor}" em "${item.nome}": ${resps.join(' / ')} — mantendo "${SEM_RESPONSAVEL}".`);
      return SEM_RESPONSAVEL;
    }
    // empresa encontrada mas sem responsavel_atual definido → para por aqui
    return SEM_RESPONSAVEL;
  }
  return SEM_RESPONSAVEL; // nenhum identificador casou
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // settings: check for updates (real, via electronAPI)
  const updBtn = $('btn-check-update');
  if (updBtn && window.electronAPI && window.electronAPI.checkForUpdates) {
    updBtn.addEventListener('click', async () => {
      updBtn.textContent = 'Verificando…'; updBtn.disabled = true;
      try {
        const r = await window.electronAPI.checkForUpdates();
        $('update-last-check').textContent = 'agora há pouco';
        if (r && r.success) {
          if (r.latestVersion && r.latestVersion !== r.currentVersion) {
            showToast(`Atualização ${r.latestVersion} encontrada — baixando…`, 'info');
          } else {
            showToast(`Você está na versão mais recente (${r.currentVersion}).`, 'ok');
          }
        } else {
          showToast('Falha ao verificar: ' + (r && r.error ? r.error : 'desconhecido'), 'error');
        }
      } catch (e) {
        showToast('Erro ao verificar atualizações.', 'error');
      } finally {
        updBtn.textContent = 'Verificar agora'; updBtn.disabled = false;
      }
    });
  } else if (updBtn) {
    updBtn.addEventListener('click', () => {
      updBtn.textContent = 'Verificando…';
      setTimeout(() => { updBtn.textContent = 'Verificar agora'; showToast('Modo demonstração — sem auto-update.', 'info'); }, 800);
    });
  }

  // status do auto-update em tempo real
  if (window.electronAPI && window.electronAPI.onUpdateStatus) {
    window.electronAPI.onUpdateStatus((d) => {
      if (!d) return;
      if (d.status === 'available' && d.info) showToast(`Atualização ${d.info.version} disponível — baixando…`, 'info');
      if (d.status === 'downloading' && d.info) { const el = $('update-last-check'); if (el) el.textContent = `baixando ${d.info.percent}%`; }
      if (d.status === 'downloaded' && d.info) showToast(`Atualização ${d.info.version} pronta. Reinicie para instalar.`, 'ok');
      if (d.status === 'error' && d.info) showToast('Erro no update: ' + (d.info.message || ''), 'error');
    });
  }

  // preenche a versão REAL do app na UI (sidebar, rodapé, card de atualização)
  if (window.electronAPI && window.electronAPI.getAppVersion) {
    window.electronAPI.getAppVersion().then((v) => {
      ['sidebar-version', 'footer-version'].forEach((id) => { const el = $(id); if (el) el.textContent = 'SPEEDSPED v' + v; });
      const card = document.querySelector('.update-card div[style*="font-weight:600"]');
      if (card) card.textContent = `Versão ${v} — instalada`;
    }).catch(() => {});
  }
...
function renderFilaCompanies() {
  ...
  const empIdx = buildEmpresaIndex();
  body.innerHTML = rows.map((e) => {
    const [sc, sl] = stMap[e.status] || ['idle', e.status];
    const resp = resolveResponsavel(e, empIdx);
    ...
    return `<tr>
      ...
      <td><span class="resp-chip"><span class="resp-avatar">${initials(resp)}</span>${escapeHTML(resp == null ? '?' : resp)}</span></td>
...
function buildHistoryItems() {
  let items = [];
  const idx = buildEmpresaIndex();
  if (cachedSubfolders && cachedSubfolders.length) {
    cachedSubfolders.forEach((folder) => {
      let g = null, gid = null;
      if (cachedGerados) for (const [id, rec] of Object.entries(cachedGerados)) if (rec.nome && rec.nome.toLowerCase() === folder.name.toLowerCase()) { g = rec; gid = id; break; }
      let lastMod = '';
      if (folder.files.length) lastMod = folder.files.reduce((a, b) => new Date(a.mtime) > new Date(b.mtime) ? a : b).mtime;
      const it = { nome: folder.name, path: folder.path, files: folder.files, fileCount: folder.fileCount, totalSize: folder.totalSize, status: g ? (g.status || 'concluido') : 'concluido', responsavel: g ? g.responsavel : '', empresaId: gid, nomeBase: g ? g.nome_base : '', cnpj: g ? g.cnpj : '', tipo: g ? g.tipo : 'Fiscal', motivo: g ? g.motivo : '', dataGeracao: (g && g.data_geracao) || lastMod, ultimoDownload: g ? g.ultimo_download : '' };
      it.responsavel = resolveResponsavel(it, idx);
      items.push(it);
    });
  } else if (cachedGerados) {
    for (const [id, r] of Object.entries(cachedGerados)) {
      const it = { nome: r.nome || '—', path: '', files: (r.arquivos || []).map((fp) => ({ name: fp.split(/[\\/]/).pop(), path: fp, size: 0 })), fileCount: (r.arquivos || []).length, totalSize: 0, status: r.status || 'concluido', responsavel: r.responsavel, empresaId: id, nomeBase: r.nome_base || '', cnpj: r.cnpj || '', tipo: r.tipo || 'Fiscal', motivo: r.motivo || '', dataGeracao: r.data_geracao || '', ultimoDownload: r.ultimo_download };
      it.responsavel = resolveResponsavel(it, idx);
      items.push(it);
    }
  }
...
function renderHistoryTable() {
  ...
  body.innerHTML = rows.map((it) => {
    ...
    return `<tr class="${isToday ? 'today' : ''}">
      ...
      <td><span class="resp-chip"><span class="resp-avatar">${initials(it.responsavel)}</span>${escapeHTML(it.responsavel == null ? '?' : it.responsavel)}</span></td>
